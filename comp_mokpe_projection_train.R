comp_mokpe_projection_train <- function(K_c, K_x, K_z, X, Z, parameters) {
  state_path <- "./comparison_states"
  start_time <- Sys.time()
  N_x <- dim(K_x)[1]
  D_x <- dim(K_x)[2]
  N_z <- dim(K_z)[1]
  ...
  solver.params <- get.solver.params(DEBUG = 0, Tolerance = 5e-3)
  ...
  for (iter in 1:100) {
    ...
    if (iter %% 5 == 0) {
      ...
      save("state", file = sprintf("%s/%s/R_%s/%s_%s/comp_old_%s_R_%s_dk_%s_tk_%s_fold_%s_iter_%s_state.RData", state_path, network, R, dk, tk, network, R, dk, tk, f, iter))
    }
  }
}
