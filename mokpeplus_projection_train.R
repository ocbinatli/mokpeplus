mokpeplus_projection_train <- function(K_c, K_x, K_z, X, Z, parameters) {
  start_time <- Sys.time()
  N_x <- dim(K_x)[1]
  D_x <- dim(K_x)[2]
  N_z <- dim(K_z)[1]
  ...
  for (iter in 1:iter_num) {
    ...
    if (iter %% 5 == 0) {
      ...
      save("state", file = sprintf("%s/mokpeplus_old_%s_R_%s_num_%s_iter_%s_state.RData", result_path, network, R, numb, iter))
    }
  }
  state <- list(Q_x = Q_x, Q_z = Q_z, sigma_e = sigma_e, objective_c = objective_c_all, objective_x = objective_x_all, objective_z = objective_z_all, objective = objective_all, time = timediff_sec)
}
